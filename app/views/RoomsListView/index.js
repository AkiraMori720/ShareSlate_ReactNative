...
import { HeaderItem, HeaderIconButton } from './Header/HeaderItem';
import ShareSlate from '../../lib/shareslate';
import RoomShareItem from '../../presentation/RoomShareItem';
import { searchShareContacts, searchShareGroups } from '../../Test/data';
import ShareSearchBox from './ShareSearchBox';
...
const HEADER_TAB_GROUPS = 'groups';
const HEADER_TAB_CONTACT = 'contact';

const HEADER_SUB_TAB_MY_CONTACT = 'my_contact';
const HEADER_SUB_TAB_SENT_REQUEST = 'sent_request';
const HEADER_SUB_TAB_RECEIVED_REQUEST = 'received_request';
...
class RoomsListView extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			chats: [],
			item: {},
			headerTab : HEADER_TAB_CONTACT,
			headerSubTab: HEADER_SUB_TAB_MY_CONTACT
		};
		this.setHeader();
		this.init();
	}
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { loadingServer, searchText, server, shareUser } = this.props;
	...
	shouldComponentUpdate(nextProps, nextState) {
		const { chatsUpdate, searching, isDirectory, item, headerTab, headerSubTab } = this.state;
		...
		if (nextState.headerTab !== headerTab) {
			return true;
		}

		if (nextState.headerSubTab !== headerSubTab) {
			return true;
		}

		if (nextState.isDirectory !== isDirectory) {
			return true;
		}
		...
		const { rooms, width, insets, shareUser } = this.props;
		...
		if (!isEqual(nextState.shareSearch, shareSearch)) {
			return true;
		}
		if (!isEqual(nextState.directories, directories)) {
			return true;
		}
	...
	init = async () => {
		const result = await ShareSlate.search({ text:'' });
		if(this.mounted){
			this.setState({ shareSearch: result });
		} else {
			this.state.shareSearch = result;
		}
	}
	...
	getHeader = () => {
		const { searching, isDirectory } = this.state;
		...
		return {
			...
			headerLeft: () => ((searching || isDirectory) ? (
	...
	initSearching = () => {
		...
		this.internalSetState({ searching: true, isDirectory: false }, () => {
			openSearchHeader();
			this.setHeader();
		});
	};
	...
	cancelSearch = () => {
		...
		if (!searching && !isDirectory) {
			return;
		}

		Keyboard.dismiss();

		this.setState({ searching: false, isDirectory: false, search: [] }, () => {
			this.setHeader();
			closeSearchHeader();
			setTimeout(() => {
				this.scrollToTop();
	...
	initDirectory = () => {
		const { openSearchHeader } = this.props;
		this.internalSetState({ isDirectory: true, searching: false }, () => {
			openSearchHeader();
			this.setHeader();
		});
	};
	...
	onShareSlateSearch = debounce(async(text) => {
		const result = await ShareSlate.search({ text });
		...
		return (
			<View style={styles.searchHeader}>
				<View style={styles.headerRow}>
					<Text style={styles.headerTitle}>Search Share Slate</Text>
					<ShareSearchBox
						onChange={this.onShareSlateSearch}
						theme={theme}
					/>
				</View>
		...
		return (
			<View style={styles.directoryHeader}>
				...
				<View style={styles.headerTabRow}>
					<HeaderItem
						title={'My Contact'}
						onPress={() => { this.setState({ headerSubTab: HEADER_SUB_TAB_MY_CONTACT })}}
						isActive={headerSubTab === HEADER_SUB_TAB_MY_CONTACT}
						theme={theme}
					/>
					<HeaderItem
						title={'Sent Request'}
						onPress={() => { this.setState({ headerSubTab: HEADER_SUB_TAB_SENT_REQUEST })}}
						isActive={headerSubTab === HEADER_SUB_TAB_SENT_REQUEST}
						theme={theme}
					/>
					<HeaderItem
						title={'Received Request'}
						onPress={() => { this.setState({ headerSubTab: HEADER_SUB_TAB_RECEIVED_REQUEST })}}
						isActive={headerSubTab === HEADER_SUB_TAB_RECEIVED_REQUEST}
						theme={theme}
					/>
				</View>
			</View>
	...
	renderHeaderButtons = () => {
		const { headerTab } = this.state;
		const { theme } = this.props;
		return (
			<View style={styles.headerBtnGroup}>
				<HeaderIconButton
					text={'Contact'}
					left={<Image source={require('../../images/icon_user.png')} style={[styles.icon, { tintColor: headerTab === HEADER_TAB_CONTACT?'white':themes[theme].grayColor }]} />}
					type='primary'
					testID='room-list-view-contact'
					theme={theme}
					style={styles.headerBtn}
					current={ headerTab === HEADER_TAB_CONTACT}
					onPress={() => { this.setState({ headerTab: HEADER_TAB_CONTACT })}}
					/>
				<HeaderIconButton
					text={'Groups'}
					left={<Image source={require('../../images/icon_group.png')} style={[styles.icon, { tintColor: headerTab === HEADER_TAB_GROUPS?'white':themes[theme].grayColor }]} />}
					type='primary'
					testID='room-list-view-group'
					theme={theme}
					style={styles.headerBtn}
					current={ headerTab === HEADER_TAB_GROUPS}
					onPress={() => { this.setState({ headerTab: HEADER_TAB_GROUPS })}}
					/>
			</View>
		);
	}
	...
	renderListHeader = () => {
		const { searching, isDirectory } = this.state;
		const {
			sortBy, queueSize, inquiryEnabled, encryptionBanner, user, theme
		} = this.props;
		return (
			<>
				<ListHeader
					...
					goDirectory={this.initDirectory}
					queueSize={queueSize}
				...
				/>
				{
					searching? this.renderSearchHeader() : null
				}
				{
					isDirectory?this.renderDirectoryHeader() : null
				}
			</>
		);
	};
	...
	renderShareItem = ({ item }) => {
		const { searching, headerTab } = this.state;
		const { shareUser, theme } = this.props;
		const id = searching?item.user_id:item.id;
		return (
			<RoomShareItem
				id={id}
				shareUserId={shareUser.user_id}
				item={item}
				isFriend={searching && shareUser.friends && shareUser.friends.includes(id)}
				onPress={()=>{}}
				onChat={() => {}}
				isSearching={searching}
				isContact={ headerTab === HEADER_TAB_CONTACT }
				theme={theme}
			/>
		);
	};
	...
	renderScroll = () => {
		const {
			loading, chats, shareSearch, directories, searching, isDirectory, headerTab, headerSubTab
		} = this.state;
		const { theme, refreshing, requests } = this.props;
		...
		let data = [];
		if(searching){
			data = headerTab === HEADER_TAB_CONTACT? shareSearch:searchShareGroups;
		} else if(isDirectory){
			if(headerTab === HEADER_TAB_CONTACT){
				switch (headerSubTab) {
					case HEADER_SUB_TAB_MY_CONTACT:
						break;
					case HEADER_SUB_TAB_SENT_REQUEST:
						data = requests.filter(request => request.type === FRIEND_REQUEST_TYPE_SENT);
						break;
					case HEADER_SUB_TAB_RECEIVED_REQUEST:
						data = requests.filter(request => request.type === FRIEND_REQUEST_TYPE_RECEIVED);
						break;
					default:
						break;
				}
		...
		if(searching){
			return (
				<FlatList
					...
					data={data}
					extraData={data}
					keyExtractor={item => HEADER_TAB_CONTACT?item.user_id:item.id}
