...
class SelectedUsersView extends React.Component {
	...
	shouldComponentUpdate(nextProps, nextState) {
		...
		const { selectUsers, users, loading, theme } = this.props;
		...
		if (!equal(nextProps.selectUsers, selectUsers)) {
			return true;
		}
	...
	componentDidUpdate(prevProps) {
		if (this.isGroupChat()) {
			...
			if (prevProps.selectUsers.length !== selectUsers.length) {
				this.setHeader(selectUsers.length > 0);
			}
		}
	}
	...
	search = async(text) => {
		...
		const result = chats.filter(user => user.username.includes(text));
		this.setState({
			search: result
		});
	}
	...
	isChecked = (username) => {
		const { selectUsers } = this.props;
		return selectUsers.findIndex(el => el.name === username) !== -1;
	}

	toggleUser = (user) => {
		const { maxUsers } = this.state;
		const {
			addUser, removeUser, selectUsers, user: { username }
		} = this.props;
		...
		if (!this.isChecked(user.name)) {
			if (this.isGroupChat() && selectUsers.length === maxUsers) {
	...
	_onPressItem = (id, item = {}) => {
		if (item.search) {
			...
			this.toggleUser({ _id: item._id, name: item.name, fname: item.username });
		}
	}
	...
	renderSelected = () => {
		const { selectUsers, theme } = this.props;
	...
	renderItem = ({ item, index }) => {
		...
		const name = item.search ? item.name : item.username;
...
const mapStateToProps = state => ({
	...
	selectUsers: state.selectedUsers.users,
