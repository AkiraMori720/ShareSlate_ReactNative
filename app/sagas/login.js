...
import {
	loginFailure, loginSuccess, setUser, logout, setShareUser
} from '../actions/login';
...
const logoutShareSlate = () => ShareSlate.logout();
const fetchProfileCall = ({ user_id }) => ShareSlate.fetchProfile({ user_id });

const handleLoginRequest = function* handleLoginRequest({ credentials, logoutOnError = false }) {
	logEvent(events.LOGIN_DEFAULT_LOGIN);
	try {
		let result;
		if (credentials.resume && credentials.ss_user) {
			result = yield call(loginCall, credentials);
		} else {
			result = yield call(loginWithPasswordCall, credentials);
		}

		const { ss_user, ...user } = result;
		let shareslate_user = yield call(fetchProfileCall, ss_user);

		if(!shareslate_user){
			yield put(loginFailure({ success: false, data: {message: 'Fetching user data failed!'}}));
			return;
		}

		shareslate_user = { ...shareslate_user, token: ss_user.token };

		if (!shareslate_user.fname || !shareslate_user.lname) {
			yield put(serverFinishAdd());
			yield put(setUser(user));
			yield put(setShareUser(shareslate_user));
			...
			yield put(loginSuccess(user, shareslate_user));
		}
	} catch (e) {
		console.log('error', e);
		if (logoutOnError && (e.data && e.data.message && /you've been logged out by the server/i.test(e.data.message))) {
			...
			yield put(loginFailure(e.data?e:{ success: false, data: {message: 'Credential is not correct'}}));
		}
	}
};
...
const handleLoginSuccess = function* handleLoginSuccess({ user, shareUser }) {
	try {
		...
		yield put(fetchRequests(shareUser.user_id));
		...
		const su = {
			...
			fname: shareUser.fname,
			lname: shareUser.lname,
			email: shareUser.email,
			username: shareUser.username,
			contact: shareUser.contact,
			country_code: shareUser.country_code,
			gender: shareUser.gender,
			dob: shareUser.dob,
			profile_img: shareUser.profile_img,
			color: shareUser.color,
			country: shareUser.country,
			state: shareUser.state,
			city: shareUser.city,
			designation: shareUser.designation,
			school: shareUser.school,
			college: shareUser.college,
			employer: shareUser.employer,
			friends: shareUser.friends
		};
		yield serversDB.action(async() => {
			try {
				...
				await shareUsersCollection.create((record) => {
					record._raw = sanitizedRaw({  id: su.user_id }, shareUsersCollection.schema);
					Object.assign(record, su);
				});
			}
		});
		...
		yield UserPreferences.setStringAsync(ShareSlate.USER_ID_KEY, shareUser.user_id);
		yield UserPreferences.setStringAsync(ShareSlate.TOKEN_KEY, shareUser.token);
		yield put(setUser(user));
		yield put(setShareUser(shareUser));
...
const handleAddFriend = function * handleAddFriend({id}){
	const shareUserId = yield select(state => state.login.shareUser.user_id);
	const serversDB = database.servers;
	const shareUsersCollection = serversDB.collections.get('shareslate_users');
	try {
		yield serversDB.action(async() => {
			const userRecord = await shareUsersCollection.find(shareUserId);
			await userRecord.update((record) => {
				record.friends = [...record.friends, id];
			});
		});
	} catch (e) {
...
const root = function* root() {
	...
	yield takeLatest(types.USER.SHARE_ADD_FRIEND, handleAddFriend);
