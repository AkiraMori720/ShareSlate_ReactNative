...
const handleOpen = function* handleOpen({ params }) {
	...
	const [server, user, ss_user_id, ss_user_token] = yield all([
		UserPreferences.getStringAsync(RocketChat.CURRENT_SERVER),
		UserPreferences.getStringAsync(`${ RocketChat.TOKEN_KEY }-${ host }`),
		UserPreferences.getStringAsync(ShareSlate.USER_ID_KEY),
		UserPreferences.getStringAsync(RocketChat.TOKEN_KEY)
	]);
	...
	if (server === host && user) {
		...
		if (params.token) {
			...
			yield RocketChat.connect({ server: host, user: { token: params.token }, ss_user: {user_id: ss_user_id, token: ss_user_token } });
			yield take(types.LOGIN.SUCCESS);
			yield navigate({ params });
		} else {
