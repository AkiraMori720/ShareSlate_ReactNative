...
const restore = function* restore() {
	try {
		const { token, server, ss_user_id, ss_token } = yield all({
			token: UserPreferences.getStringAsync(RocketChat.TOKEN_KEY),
			server: UserPreferences.getStringAsync(RocketChat.CURRENT_SERVER),
			ss_user_id: UserPreferences.getStringAsync(ShareSlate.USER_ID_KEY),
			ss_token: UserPreferences.getStringAsync(RocketChat.TOKEN_KEY)
		});

		if (!token || !server || !ss_user_id || !ss_token) {
			yield all([
