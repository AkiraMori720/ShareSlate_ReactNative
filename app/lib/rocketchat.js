...
import { IS_DEV, SHARESLATE_MAIN_SERVER } from '../constants/server';
import moment from 'moment';
import ShareSlate from './shareslate';
...
const RocketChat = {
	...
	connect({ server, user, ss_user, logoutOnError = false }) {
			...
			const sdkConnect = () => this.sdk.connect()
				.then(() => {
					...
					if (user && user.token && server === currentServer) {
						reduxStore.dispatch(loginRequest({ resume: user.token, ss_user}, logoutOnError));
					}
				})
				.catch((err) => {
	...
	async register(credentials) {
		...
		const {username, password, email, phone, gender, dob} = credentials
		...
		formData.append("conf-password", password);
		formData.append("terms", 'on');

		const result = await fetch(`${ SHARESLATE_MAIN_SERVER }/apis/register.php`, {
			method: 'POST',
			body: formData,
		})
	...
	async loginWithPassword({ user, password }) {
		...
		const result = await ShareSlate.loginWithPassword({ user, password });
		if(result.success){
			params = {
				user: result.email,
				password: result.password,
				ss_user: result.ss_user
			}
			return this.loginTOTP(params, true);
		}
		throw result.msg;
	...
	async login(params, loginEmailPassword) {
		const sdk = this.shareSDK || this.sdk;
		// RC 0.64.0
		const {  ss_user, ...credential } = params;
		await sdk.login(credential);
	...
	getUsers({ emails, sort }){
		return this.post('users.fetch', { emails, sort });
	},
	addUsersToRoom(rid) {
	...
	inviteByEmails(emails, message){
		return this.post('inviteByEmails', {emails, message});
	}
};
